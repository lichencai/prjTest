...
public class ServerPoint {
	...
	public static void main(String[] args){
		ServerPoint xiaona = new ServerPoint();
        xiaona.init();
        xiaona.listen();
    }
	...
	private void init(){
		...
		try{
			...
			server.configureBlocking(false);	//  必须设置为非阻塞
			server.socket().bind(new InetSocketAddress(8383));
	...
	private void handleKey(SelectionKey key) throws IOException, ClosedChannelException {
        ...
        try{
            if(key.isAcceptable()){
                ...
                System.out.println("client channel system address:" + channel.hashCode());
                ...
                if(count > 0){
                    ...
                    */
                    ...
                    channel.close();            
                    System.out.println("close the receive channel...");
                }                      
            }
        }catch(Throwable t){
            t.printStackTrace();
